...
def display_ad_strategy(df, keyword_input, api_service):
    ...
    with col_a:
        ...
        st.metric(label="Classified Keywords", value=f"{classified_total_percent}%")
        st.metric(label="Transactional Intent", value=f"{intent_data['transactional']}%")
        st.metric(label="Commercial Intent", value=f"{intent_data['commercial']}%")
        st.metric(label="Informational Intent", value=f"{intent_data['informational']}%")
        st.metric(label="Unclassified Keywords", value=f"{intent_data['unclassified']}")
...
if generate_button:
    if not keyword_input:
        st.warning("Please enter a  keyword to begin the analysis.")
    else:
        ...
        if keywords_set:
            ...
            if not final_df.empty:
                ...
                st.info(f"No related keywords found from {api_source.upper()} that contain the  keyword.")